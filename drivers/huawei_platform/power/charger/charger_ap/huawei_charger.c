...
#define RBAT_CALC_SOC_INTERVAL (3)
struct rbat_over_standard_record rbat_over_standard_record_data;

#define CFG_CUR_RBAT_RATIO_TENTH	(3)
#define CFG_SOC_BEGIN			(35)
#define CFG_SOC_END			(55)
#define LOAD_TIME			(400)
#define RELEASE_TIME			(400)
#define INVALID_RBAT			(1000)
...
void init_rbat_calc_records(void)
{
	int i = 0;
	int chrg_rbat_over_standard = hisi_get_bad_battery_flag();

	memset(&rbat_over_standard_record_data, 0, sizeof(rbat_over_standard_record_data));
	rbat_over_standard_record_data.rbat_over_standard = chrg_rbat_over_standard;

	hwlog_info("%s:rbat_over_standard %d \n", __func__, chrg_rbat_over_standard);

	 for (i=0; i<RBAT_RECORD_LEN; i++) {
		rbat_over_standard_record_data.rbat_records[i] = chrg_rbat_over_standard;
	}
}

static int calc_rbat_set_current(struct charge_device_info *di, int set_curr)
{
	int i;
	int curr_now = 0;

	if (NULL == di || 0 > set_curr) {
		hwlog_err("%s:invalid input!\n", __func__);
		return 1;
	}
	for( i = 0; i <= 3; i++) {
		di->ops->set_charge_current(set_curr);
		msleep(200);
		curr_now = -hisi_battery_current();
		if (curr_now >= set_curr - RBAT_CHECK_DELTA_CURRENT && curr_now <= set_curr + RBAT_CHECK_DELTA_CURRENT) {
			return 0;
		} else if(3 == i) {
			hwlog_info("%s:set charge current to %d failed, curr_now %d !!\n", __func__, set_curr, curr_now);
			return 1;
		}
	}
	return 0;
...
void calc_chg_rbat(struct charge_device_info *di)
{
	int i = 0 ;
	int rbat_calc = 0;
	int sum_vbat_open = 0;
	...
	int sum_cur_chrg = 0 ;
	int sum_standard = 0;
	static unsigned int last_calc_soc = 0;
	int capacity = hisi_battery_capacity();
	int tbatt = hisi_battery_temperature();
	int curr_now = -hisi_battery_current();
	int cfg_cur_rbat = CFG_CUR_RBAT_RATIO_TENTH * hisi_battery_fcc_design()/10;
	int refer_rbat = 0;
	static int index = 0;
	int ret = 0;

	if (0 == di->rbat_check_enable)
		return;

	if (capacity < CFG_SOC_BEGIN  || capacity > CFG_SOC_END){
		hwlog_info("%s:capacity %d is out of [%d,%d]\n", __func__,capacity,CFG_SOC_BEGIN,CFG_SOC_END);
		return;
	}

	if (capacity < last_calc_soc + RBAT_CALC_SOC_INTERVAL && capacity >= last_calc_soc) {
		return;
	}

	if (curr_now <= cfg_cur_rbat  + RBAT_CHECK_DELTA_CURRENT || di->charge_current <=  cfg_cur_rbat + RBAT_CHECK_DELTA_CURRENT) {
		hwlog_info("%s:charge current %d  too small for test \n",  __func__, curr_now );
		return;
	}

	if(tbatt < BATTERY_TEMPERATURE_10_C){
		return;
	}else if(tbatt < BATTERY_TEMPERATURE_20_C){
		refer_rbat = di->refer_rbat_10_degree;
	}else if(tbatt < BATTERY_TEMPERATURE_30_C){
		refer_rbat = di->refer_rbat_20_degree;
	}else{
		refer_rbat = di->refer_rbat_30_degree;
	}
	/* set charge current to a certain value */
	ret = calc_rbat_set_current(di, cfg_cur_rbat);
	if (ret) {
		return;
	}
	msleep(LOAD_TIME);
	for( i = 0; i <= 3; i++) {
		sum_vbat_chrg +=  hisi_battery_voltage();
		sum_cur_chrg  +=  -hisi_battery_current();
		msleep(200);
		hwlog_info(" %s: index %d,sum_vbat_chrg %d,sum_cur_chrg %d \n", __func__, i,sum_vbat_chrg,sum_cur_chrg);
	}

	/* set charge current to 0 */
	ret = calc_rbat_set_current(di, 0);
	if (ret) {
		return;
	}
	msleep(RELEASE_TIME);
	for( i = 0; i <= 3; i++)
	{
		sum_vbat_open +=  hisi_battery_voltage();
		sum_cur_open  +=  -hisi_battery_current();
		msleep(200);
		hwlog_info(" %s: index %d,sum_vbat_open %d,sum_cur_open %d \n", __func__,i,sum_vbat_open,sum_cur_open);
	}

	/* calculate battery resistance */
	rbat_calc =  ( sum_vbat_open - sum_vbat_chrg) * 1000 / (sum_cur_open-sum_cur_chrg);

	i = index % RBAT_RECORD_LEN;
	if(rbat_calc > refer_rbat){
		rbat_over_standard_record_data.rbat_records[i] = 1;
	} else {
		rbat_over_standard_record_data.rbat_records[i] = 0;
	}
	index = i + 1;
	for (i =0; i < RBAT_RECORD_LEN;i++)
	{
		sum_standard += rbat_over_standard_record_data.rbat_records[i] ;
		hwlog_info(" %s : index %d,standard %d,sum_standard %d\n", __func__, i, rbat_over_standard_record_data.rbat_records[i],sum_standard);
	}
	if(sum_standard >= RBAT_OVER_RANGE){
		rbat_over_standard_record_data.rbat_over_standard =1;
	}else{
		rbat_over_standard_record_data.rbat_over_standard =0;
	}
	last_calc_soc = capacity;
	hisi_save_chrg_rbat_over_standard(rbat_over_standard_record_data.rbat_over_standard);
	hwlog_info("%s :number %d,tbatt %d, soc %d, rabt_calc %d refer_rbat %d rbat_over_standard %d\n",
		__func__,index,tbatt,capacity,rbat_calc,refer_rbat,rbat_over_standard_record_data.rbat_over_standard);
}
/**********************************************************
...
static void charge_update_charger_type(struct charge_device_info *di)
{
	...
	} else if (!fsa9685_dcd_timeout_enabled){
		return;
	}

	if(di->sw_ops && di->sw_ops->get_charger_type) {
		type = di->sw_ops->get_charger_type();
		if (fsa9685_dcd_timeout_enabled) {
			if (CHARGER_TYPE_NONE == type &&  (CHARGER_TYPE_NON_STANDARD != di->charger_type &&
					CHARGER_TYPE_USB != di->charger_type) ) {
				/* switch detect error, report DMD and print log*/
				hwlog_info("[%s]charger type is [%d]\n",__func__, di->charger_type);
				charger_dsm_report(ERROR_SCHARGERV200_FAULT, NULL);
			}
			if (CHARGER_TYPE_NON_STANDARD == di->charger_type && CHARGER_TYPE_DCP == type) {
...
int set_charger_disable_flags(int val, int type)
{
	...
	di->sysfs_data.disable_charger[type] = (unsigned int)val;
				  ...
				  const char *buf, size_t count)
{
	...
	case CHARGE_SYSFS_ENABLE_CHARGER:
		if ((strict_strtol(buf, 10, &val) < 0) || (val < 0) || (val > 1))
			return -EINVAL;
		set_charger_disable_flags(val?CHARGER_CLEAR_DISABLE_FLAGS:CHARGER_SET_DISABLE_FLAGS,
			CHARGER_SYS_NODE);
...
static void charge_parse_dts(struct charge_device_info *di)
{
	...
	hwlog_info("dcd_timeout_enabled = %d\n", fsa9685_dcd_timeout_enabled);
	...
	ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"),
        ...
        ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"), \
                        "refer_rbat_10_degree", &di->refer_rbat_10_degree);
        if (ret) {
                hwlog_err("get refer_rbat_10_degree fail!\n");
                di->refer_rbat_10_degree = INVALID_RBAT;
        }
        hwlog_info("refer_rbat_10_degree = %d\n", di->refer_rbat_10_degree);

        ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"), \
                        "refer_rbat_20_degree", &di->refer_rbat_20_degree);
        if (ret) {
                hwlog_err("get refer_rbat_20_degree fail!\n");
                di->refer_rbat_20_degree = INVALID_RBAT;
        }
        hwlog_info("refer_rbat_20_degree = %d\n", di->refer_rbat_20_degree);

        ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"), \
                        "refer_rbat_30_degree", &di->refer_rbat_30_degree);
        if (ret) {
                hwlog_err("get refer_rbat_30_degree fail!\n");
                di->refer_rbat_30_degree = INVALID_RBAT;
        }
        hwlog_info("refer_rbat_30_degree = %d\n", di->refer_rbat_30_degree);

        ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "hisi,coul_core"), \
                        "rbat_check_enable", &di->rbat_check_enable);
        if (ret) {
                hwlog_err("get rbat_check_enable fail!\n");
                di->rbat_check_enable = 0;
        }
        hwlog_info("rbat_check_enable = %d\n", di->rbat_check_enable);
